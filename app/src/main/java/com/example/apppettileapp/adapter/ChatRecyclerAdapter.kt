...
import com.example.apppettileapp.databinding.RecyclerRowChatBinding
...
class ChatRecyclerAdapter : RecyclerView.Adapter<ChatRecyclerAdapter.ChatHolder>() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ChatHolder {
        if(viewType == VIEW_TYPE_MESSAGE_RECEIVED) {
            val view = LayoutInflater.from(parent.context).inflate(R.layout.recycler_row_chat, parent, false)
            return ChatHolder(view)

        } else {
            val view = LayoutInflater.from(parent.context).inflate(R.layout.recycler_row_chat_right, parent, false)
            return ChatHolder(view)
    ...
    override fun onBindViewHolder(holder: ChatHolder, position: Int) {
        val textView = holder.itemView.findViewById<TextView>(R.id.messageText)
        textView.text = "${chats.get(position).userEmail} : ${chats.get(position).text}"
    }
